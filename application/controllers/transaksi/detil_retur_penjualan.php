 ...
 */
class Detil_retur_penjualan extends MY_Controller {

    private $_CFG;
    private $_TBL_PRIMARY = "retur_order_details a";
...
//        $this->_CFG->add_column("Total Price", $userName);
    ...
    function index($id_retur = "null") {
        $data = array();
        $whr = array();
        $this->session->set_userdata(array(md5(__FILE__ . "id_retur") => $id_retur));

        if ($id_po != "null") {
            $whr = $whr + array("a.retur_id" => intval($id_retur));
            $id_order = $this->base_model->list_single_data("order_id", "retur_orders", "", array("id"=>intval($id_retur)));
            $this->session->set_userdata(array(md5(__FILE__ . "id_order") => intval( $id_order->order_id)));
        }
    ...
    function form($mode, $key = "") {
        $data = array();
        $id_supplier = $this->session->userdata(md5(__FILE__ . "id_retur"));

        $dg = new Datagrid();


        if ($key == "") {
            $orderId = $this->session->userdata(md5(__FILE__ . "id_order"));
            $rsGrup = $this->base_model->list_data("a.product_id as kunci, concat_ws(' - ', b.code, b.name ) as nilai", "order_detail a", array("product b"=>"b.id=a.product_id"), array('a.order_id' => intval($orderId)), array("a.id asc"), "", "", "", FALSE);
    ...
    private function _updateStock($poDetailId = "", $refunded="") {
        // get data retur detail
        $rsPoDetails = $this->base_model->list_single_data("*", "retur_order_details", "", array("retur_detail_id" => intval($poDetailId)));

        if ($rsPoDetails != "") {
            // get data retur
            $rsPo = $this->base_model->list_single_data("*", "retur_orders", "", array("id" => intval($rsPoDetails->retur_id)));
