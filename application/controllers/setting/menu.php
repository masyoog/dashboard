...
class Menu extends MY_Controller {
    ...
    function form($mode = "add", $key = "") {
        ...
        $rsMenuInduk = $this->base_model->list_data("id as kunci, menu as nilai", "sys_menu", "", array('status' => 1, 'id_induk' => 0, 'id !='=> intval($key)), array("menu asc"));
    ...
    private function _ubah($key = "") {
        ...
        if ("" != $key && "" != $this->_CFG->get_KEYS()) {
            ...
            if (is_array($columns)) {
                ...
                $affected = $this->base_model->update_data($this->_TBL_PRIMARY, $datas, $whr);
                if ($affected > 0) {
