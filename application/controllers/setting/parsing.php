...
class Parsing extends MY_Controller {
    ...
    function __construct() {
        ...
        $grupUser->set_FIELD_DB("d.supplier_name");
        $grupUser->set_SIZE(4);
    ...
    function index($id_supplier = "null", $id_produk = "null") {
        $data = array();
        $whr = array();
        $this->session->set_userdata(array(md5(__FILE__ . "id_supplier") => $id_supplier));
        $this->session->set_userdata(array(md5(__FILE__ . "id_produk") => $id_produk));

        if ($id_supplier != "null") {
            $whr = $whr + array("a.supplier_id" => intval($id_supplier));
        }

        if ($id_produk != "null") {
            $whr = $whr + array("a.product_id" => intval($id_produk));
        }
    ...
    function form($mode, $key = "") {
        $data = array();
        $id_supplier = $this->session->userdata(md5(__FILE__ . "id_supplier"));
        ...
        if ($key == "") {
            $rsGrup = $this->base_model->list_data("id as kunci, concat_ws(' - ', code, name) as nilai", "product", "", array('status' => 1, 'id NOT IN (SELECT product_id FROM parsing WHERE supplier_id=' . intval($id_supplier) . ')' => NULL), array("name asc"), "", "", "", FALSE);
            $grupUser = new Datagridcolumn();
    ...
    private function _tambah() {
        ...
        $datas = array("supplier_id" => $id_supplier);
        if (is_array($columns)) {
