...
class Grup_akses extends MY_Controller {
    ...
    private $_ORDER = array("b.id_induk ASC", "a.sys_menu_id ASC", "a.sys_grup_user_id ASC");
    private $_ITEM_PER_PAGE = "";
    private $_TBL_JOIN = array(
        "sys_menu b" => "a.sys_menu_id=b.id",
        "sys_grup_user c" => "a.sys_grup_user_id=c.id",
    ...
    function __construct() {
        ...
        $grup->set_SIZE(128);
        $grup->set_FORM_ID("sys_grup_user_id");
    ...
    function index($id_menu = "", $id_grup = "") {
        ...
        if ("" != $id_menu && "null" != $id_menu) {
            $whr = $whr + array("a.sys_menu_id" => intval($id_menu));
            $this->session->set_userdata(array(md5(__FILE__ . "id_menu") => $id_menu));
        } else {
            $this->session->set_userdata(array(md5(__FILE__ . "id_menu") => "null"));
        }

        if ("" != $id_grup && "null" != $id_grup) {
            $whr = $whr + array("a.sys_grup_user_id" => intval($id_grup));
            $this->session->set_userdata(array(md5(__FILE__ . "id_grup") => $id_grup));
        } else {
            $this->session->set_userdata(array(md5(__FILE__ . "id_grup") => "null"));
        }
        ...
        if ($this->input->post('save') != "") {
            ...
            if ($errorMsg == "") {
                ...
                if ("" != $this->base_model->db->_error_message()) {
                    ...
                    $id_menu = $this->session->userdata(md5(__FILE__ . "id_menu"));
                    $id_grup = $this->session->userdata(md5(__FILE__ . "id_grup"));
                    redirect(base_url($this->_INDEX_PAGE . "/index/" . $id_menu . "/" . $id_grup) . _build_query_string($this->_get_query_string()));
                }
            }
        }
